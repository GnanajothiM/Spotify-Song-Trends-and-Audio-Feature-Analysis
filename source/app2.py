...
def load_data():
    ...
    file_path = os.path.join(
        ...
        "..",
        "dataset",
        "tamil_spotify_dataset_for_portfolio.xlsx"
    )
    ...
    df["playlist_subgenre"] = df["playlist_subgenre"].astype(str)
...
subgenre = st.sidebar.multiselect(
    "Playlist Category",
    sorted(spotify["playlist_subgenre"].dropna().unique()),
    default=sorted(spotify["playlist_subgenre"].dropna().unique())
)

spotify = spotify[spotify["playlist_subgenre"].isin(subgenre)]
...
with col2:
    ...
    genre = spotify["playlist_subgenre"].value_counts()
...
with col5:
    ...
    energy = (
        spotify.groupby("playlist_subgenre")["energy"]
        .mean()
    )
...
top_genre = spotify["playlist_subgenre"].value_counts().idxmax()
...
🎼 **Most Popular Playlist Category:** **{top_genre}** contains the largest number of tracks.

💿 **Highest Rated Album:** **{top_album}** has the highest average popularity.

⚡ **Highest Energy Playlist Category:** **{highest_energy}** has the highest average energy.
